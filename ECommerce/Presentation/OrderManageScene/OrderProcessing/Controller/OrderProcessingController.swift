//
//  OrderProcessingController.swift
//  ECommerce
//
...
protocol OrderProcessingControllerInput {
    func didLoad()
    func didSelectItem(at index: Int)
}

protocol OrderProcessingControllerOutput {
    var items: Observable<[OrderProcessingItemModel]> { get }
    var isEmpty: Bool { get }
    var screenTitle: String { get }
    var emptyDataTitle: String { get }
    var errorTitle: String { get }
    
    var onSelectOrderItem: ((OrderProcessingItemModel) -> Void)? { get set }
}

typealias OrderProcessingController = OrderProcessingControllerInput & OrderProcessingControllerOutput & EcoController

final class DefaultOrderProcessingController: OrderProcessingController {
    ...
    let items: Observable<[OrderProcessingItemModel]> = Observable([])
    var isEmpty: Bool { return items.value.isEmpty }
    var screenTitle: String { "order_status_processing".localized() }
    let emptyDataTitle = "No processing orders"
    var errorTitle: String { "error".localized() }
    
    var onSelectOrderItem: ((OrderProcessingItemModel) -> Void)?
    ...
    private func filterProcessingOrders() {
        let processingOrders = allOrders.filter { $0.orderStatus == "processing" }
        items.value = processingOrders.map { OrderProcessingItemModel(orderManage: $0) }
    }
